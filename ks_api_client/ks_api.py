...
class KSTradeApi():
    def __init__(self, access_token = None, userid = None, consumer_key = None, ip = None, app_id = "", host = None, proxy_url = None, \
                proxy_user = None, proxy_pass = None, consumer_secret = None,logger=True):
        error = None
        session_init = None
        if host:
            hosts = [host]
            ...
            if 'result' in jsonResponse and 'token' in jsonResponse['result'] and jsonResponse['result']['token']:
                ...
                self.sio = socketio.Client(
                    reconnection=True, request_timeout=20, reconnection_attempts=5, engineio_logger=False,
                            logger=self.logger,http_session=session, ssl_verify=session.verify)
                ...
                self.sio.connect(broadcast_host, 
                        ...
                        transports=["websocket"], socketio_path="/feed/fast")
            else:
